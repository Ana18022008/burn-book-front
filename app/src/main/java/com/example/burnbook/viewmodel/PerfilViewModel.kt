...
class PerfilViewModel(
    ...
    fun carregarPerfil(usuarioId: Long) {
        viewModelScope.launch {
            ...
            val result = usuarioRepository.buscarPorId(usuarioId)
            _perfilState.value = if (result.isSuccess) {
                PerfilState.Sucesso(result.getOrNull()!!)
            } else {
                PerfilState.Erro(result.exceptionOrNull()?.message ?: "Erro ao carregar perfil")
            }
        }
    }
        ...
        viewModelScope.launch {
            ...
            if (result.isSuccess) {
                ...
                if (estadoAtual is PerfilState.Sucesso) {
                    _perfilState.value = estadoAtual.copy(publicacoes = publicacoes.toList())
                }
            }
        }
    }
